...
public class CompanyFrame extends JFrame {
	...
	void createNewCompany(String Name,String Code) {
		_client.addCompany(Name,Code);
	}
	void updateCompanyTable(ArrayList<SomeClass> companys) {
		DefaultTableModel model = (DefaultTableModel) tableComp.getModel();
		model.setRowCount(0);
		companys.forEach(company ->{
				model.addRow(new Object[]{company.getName(),
						company.getStockCode(),
						String.valueOf(company.getPrice()),
						String.valueOf(company.getStockQuantity())
						});
		});
	}
